...
export default function ServiceCategoriesPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/service-categories', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  async function handleDelete(id: string) {
    if (!confirm('Xoá danh mục này? Các dịch vụ thuộc danh mục sẽ không bị xoá.')) return
    await fetch(`/api/admin/service-categories?id=${id}`, { method: 'DELETE' })
    fetchItems()
  }
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-center px-4 py-3 font-medium text-gray-600 w-24">Dịch vụ</th>
            ...
            <tbody className="divide-y">
              {items.map(item => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center text-gray-600">{item._count?.services ?? 0}</td>
