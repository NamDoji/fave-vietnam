...
export default function ProjectCategoriesPage() {
  ...
  function openEdit(item: ProjectCategory) {
    setEditing(item)
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/project-categories', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  async function handleDelete(id: string) {
    if (!confirm('Xoá danh mục này? Các dự án thuộc danh mục sẽ không bị xoá.')) return
    await fetch(`/api/admin/project-categories?id=${id}`, { method: 'DELETE' })
    fetchItems()
  }
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-center px-4 py-3 font-medium text-gray-600 w-24">Dự án</th>
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <FolderOpen size={14} className="text-[#00a0e9] flex-shrink-0" />
                      <div>
                  ...
                  <td className="px-4 py-3 text-center text-gray-600">{item._count?.projects ?? 0}</td>
